...
- (void)dealloc
{
	NSLog(@"Dealloc called - audiostreamer");
	...
	@synchronized(self)
	{
        ...
        if(oAuthToken)
        {
            CFHTTPMessageSetHeaderFieldValue(message, CFSTR("Authorization"),
                                             (__bridge CFStringRef)[NSString stringWithFormat:@"OAuth %@", oAuthToken]);
        }
        ...
        //CFStringRef authKey = CFDictionaryGetValue(headers, CFSTR("Authorization"));
        //NSLog(@"OAuth Header:%@", (NSString*)authKey);
        //CFRelease(headers);
...
- (void)start
{
	@synchronized (self)
	{
		...
		else if (state == AS_INITIALIZED)
		{
			...
			notificationCenter =
            [[NSNotificationCenter defaultCenter] ah_retain];
